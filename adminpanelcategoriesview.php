...
<body>
    ...
    <table class="table table-striped table-bordered mydatatable " style="width:100% " id="mydatatable">
      ...
      <tbody>
        ...
        while($row = mysqli_fetch_array($result))
        {
          ...
          <td>".$row['strCategoryThumb']."</td>
          <td>".$row['strCategoryDescription']."</td>
  ...
  <section id="about" class="pl-5 pt-5 pb-5 pr-5  background" style="width: 35%; margin-left: 18%;margin-right: auto;">
    ...
    <section id='main' style="border: solid 2px #242424 " class="mt-1 mb-1">
      <div class="hidden">
        <form id="auth" class="popup_fast py-2 pl-4 pr-4 " style="right:38%;" method="POST">
          ...
          <p></p>
          <input type="text" name="catID" oninput="this.value = this.value.replace(/[^0-9]/g, '').replace(/(\..*?)\..*/g, '$1');" size=20 placeholder="Идентификатор записи">
          <p></p>
          <button type="submit" name="action" value="deletecategory" class="btn" style="border: solid 1px; width: 34%;">Удалить запись</button>
        </form>
      </div>
    </section>
  </section>
  ...
  <?php 
  if (isset($_POST['catID']) and $_REQUEST['action'] =='deletecategory')
  {
    ...
    $queryy = "DELETE from categories WHERE idCategory = '$categoryid'";
    ...
    echo '<script>window.location="adminpanelcategoriesview.php"</script>';
